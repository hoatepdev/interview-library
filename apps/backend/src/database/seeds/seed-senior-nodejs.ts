/**
 * Seed Script for Senior-Level Node.js Interview Questions
 *
 * Usage:
 *   npx ts-node -r tsconfig-paths/register src/database/seeds/seed-senior-nodejs.ts
 *   or: pnpm --filter backend seed:senior-nodejs
 *
 * Adds 10 senior-level Node.js questions.
...
async function runSeed() {
  console.log("🌱 Starting senior-level Node.js questions seed...");
  ...
  try {
    ...
    console.log("❓ Adding senior-level Node.js questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of seniorNodejsQuestions) {
