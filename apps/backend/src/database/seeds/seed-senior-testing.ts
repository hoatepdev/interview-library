/**
 * Seed Script for Senior-Level Testing Interview Questions
 *
 * Usage:
 *   npx ts-node -r tsconfig-paths/register src/database/seeds/seed-senior-testing.ts
 *   or: pnpm --filter backend seed:senior-testing
 *
 * Adds 10 senior-level Testing questions.
...
async function runSeed() {
  console.log("🌱 Starting senior-level Testing questions seed...");
  ...
  try {
    ...
    console.log("❓ Adding senior-level Testing questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of seniorTestingQuestions) {
