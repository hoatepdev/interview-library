/**
 * Seed Script for Senior-Level NestJS Interview Questions
 *
 * Usage:
 *   npx ts-node -r tsconfig-paths/register src/database/seeds/seed-senior-nestjs.ts
 *   or: pnpm --filter backend seed:senior-nestjs
 *
 * Adds 10 senior-level NestJS questions.
...
async function runSeed() {
  console.log("🌱 Starting senior-level NestJS questions seed...");
  ...
  try {
    ...
    console.log("❓ Adding senior-level NestJS questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of seniorNestjsQuestions) {
