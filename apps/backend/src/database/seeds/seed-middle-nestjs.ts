/**
 * Seed Script for Middle-Level NestJS Interview Questions
 *
 * Usage:
 *   npx ts-node -r tsconfig-paths/register src/database/seeds/seed-middle-nestjs.ts
 *   or: pnpm --filter backend seed:middle-nestjs
 *
 * Adds 20 middle-level NestJS questions.
...
import { middleNestjsQuestions } from "./interview-data-middle-nestjs";

const topicSlugToId: Record<string, string> = {};

async function runSeed() {
  console.log("🌱 Starting middle-level NestJS questions seed...");
  ...
  try {
    ...
    console.log("❓ Adding middle-level NestJS questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of middleNestjsQuestions) {
