/**
 * Seed Script for Senior-Level TypeScript Interview Questions
 *
 * Usage:
 *   npx ts-node -r tsconfig-paths/register src/database/seeds/seed-senior-typescript.ts
 *   or: pnpm --filter backend seed:senior-typescript
 *
 * Adds 10 senior-level TypeScript questions.
...
async function runSeed() {
  console.log("🌱 Starting senior-level TypeScript questions seed...");
  ...
  try {
    ...
    console.log("❓ Adding senior-level TypeScript questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of seniorTypescriptQuestions) {
