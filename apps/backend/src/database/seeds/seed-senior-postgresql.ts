/**
 * Seed Script for Senior-Level PostgreSQL Interview Questions
 *
 * Usage:
 *   npx ts-node -r tsconfig-paths/register src/database/seeds/seed-senior-postgresql.ts
 *   or: pnpm --filter backend seed:senior-postgresql
 *
 * Adds 10 senior-level PostgreSQL questions.
...
async function runSeed() {
  console.log("🌱 Starting senior-level PostgreSQL questions seed...");
  ...
  try {
    ...
    console.log("❓ Adding senior-level PostgreSQL questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of seniorPostgresqlQuestions) {
