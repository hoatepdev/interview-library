/**
 * Seed Script for Senior-Level React Interview Questions
 *
 * Usage:
 *   npx ts-node -r tsconfig-paths/register src/database/seeds/seed-senior-react.ts
 *   or: pnpm --filter backend seed:senior-react
 *
 * Adds 10 senior-level React questions.
...
async function runSeed() {
  console.log("🌱 Starting senior-level React questions seed...");
  ...
  try {
    ...
    console.log("❓ Adding senior-level React questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of seniorReactQuestions) {
