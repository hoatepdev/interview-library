/**
 * Seed Script for Middle-Level Next.js Interview Questions
 *
 * Usage:
 *   npx ts-node -r tsconfig-paths/register src/database/seeds/seed-middle-nextjs.ts
 *   or: pnpm --filter backend seed:middle-nextjs
 *
 * Adds 20 middle-level Next.js questions.
...
import { middleNextjsQuestions } from "./interview-data-middle-nextjs";

const topicSlugToId: Record<string, string> = {};

async function runSeed() {
  console.log("🌱 Starting middle-level Next.js questions seed...");
  ...
  try {
    ...
    console.log("❓ Adding middle-level Next.js questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of middleNextjsQuestions) {
