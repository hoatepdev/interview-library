/**
 * Seed Script for Middle-Level Git Interview Questions
 *
 * Usage: pnpm --filter backend seed:middle-git
 *
 * Creates the "git" topic if it doesn't exist,
...
import { middleGitQuestions, newTopic } from "./interview-data-middle-git";

async function runSeed() {
  console.log("🌱 Starting middle-level Git questions seed...");
  ...
  try {
    ...
    console.log("❓ Adding middle-level Git questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of middleGitQuestions) {
