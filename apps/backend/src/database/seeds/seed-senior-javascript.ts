/**
 * Seed Script for Senior-Level JavaScript Interview Questions
 *
 * Usage:
 *   npx ts-node -r tsconfig-paths/register src/database/seeds/seed-senior-javascript.ts
 *   or: pnpm --filter backend seed:senior-javascript
 *
 * Adds 10 senior-level JavaScript questions.
...
async function runSeed() {
  console.log("🌱 Starting senior-level JavaScript questions seed...");
  ...
  try {
    ...
    console.log("❓ Adding senior-level JavaScript questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of seniorJavascriptQuestions) {
