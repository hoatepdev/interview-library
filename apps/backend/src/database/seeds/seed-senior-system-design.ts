/**
 * Seed Script for Senior-Level System Design Interview Questions
 *
 * Usage:
 *   npx ts-node -r tsconfig-paths/register src/database/seeds/seed-senior-system-design.ts
 *   or: pnpm --filter backend seed:senior-system-design
 *
 * Adds 10 senior-level System Design questions.
...
async function runSeed() {
  console.log("🌱 Starting senior-level System Design questions seed...");
  ...
  try {
    ...
    console.log("❓ Adding senior-level System Design questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of seniorSystemDesignQuestions) {
