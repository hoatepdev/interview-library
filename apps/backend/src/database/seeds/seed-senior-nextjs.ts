/**
 * Seed Script for Senior-Level Next.js Interview Questions
 *
 * Usage:
 *   npx ts-node -r tsconfig-paths/register src/database/seeds/seed-senior-nextjs.ts
 *   or: pnpm --filter backend seed:senior-nextjs
 *
 * Adds 10 senior-level Next.js questions.
...
async function runSeed() {
  console.log("🌱 Starting senior-level Next.js questions seed...");
  ...
  try {
    ...
    console.log("❓ Adding senior-level Next.js questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of seniorNextjsQuestions) {
