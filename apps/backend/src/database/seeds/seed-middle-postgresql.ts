/**
 * Seed Script for Middle-Level PostgreSQL Interview Questions
 *
 * Usage:
 *   npx ts-node -r tsconfig-paths/register src/database/seeds/seed-middle-postgresql.ts
 *   or: pnpm --filter backend seed:middle-postgresql
 *
 * Adds 20 middle-level PostgreSQL questions.
...
import { middlePostgresqlQuestions } from "./interview-data-middle-postgresql";

const topicSlugToId: Record<string, string> = {};

async function runSeed() {
  console.log("🌱 Starting middle-level PostgreSQL questions seed...");
  ...
  try {
    ...
    console.log("❓ Adding middle-level PostgreSQL questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of middlePostgresqlQuestions) {
