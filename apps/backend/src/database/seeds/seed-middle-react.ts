/**
 * Seed Script for Middle-Level React Interview Questions
 *
 * Usage:
 *   npx ts-node -r tsconfig-paths/register src/database/seeds/seed-middle-react.ts
 *   or: pnpm --filter backend seed:middle-react
 *
 * Adds 20 middle-level React questions.
...
import { middleReactQuestions } from "./interview-data-middle-react";

const topicSlugToId: Record<string, string> = {};

async function runSeed() {
  console.log("🌱 Starting middle-level React questions seed...");
  ...
  try {
    ...
    console.log("❓ Adding middle-level React questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of middleReactQuestions) {
