/**
 * Seed Script for Middle-Level Docker & DevOps Interview Questions
 *
 * Usage:
 *   pnpm --filter backend seed:middle-docker-devops
 *
 * Creates the "docker-devops" topic if it doesn't exist,
...
import {
  middleDockerDevopsQuestions,
  newTopic,
} from "./interview-data-middle-docker-devops";

async function runSeed() {
  console.log("🌱 Starting middle-level Docker & DevOps questions seed...");
  ...
  try {
    ...
    // Build topic slug -> id map (include all topics)
    ...
    console.log("❓ Adding middle-level Docker & DevOps questions...");
    let added = 0;
    let skipped = 0;

    for (const questionData of middleDockerDevopsQuestions) {
